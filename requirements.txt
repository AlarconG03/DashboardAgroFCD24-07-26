...
if refresh or "master_df" not in st.session_state:
    with st.spinner("Ejecutando pipeline de auditoría, limpieza e integración..."):
        ...
        master = integration.merge_master(inv_clean, trans_clean, fb_clean)
        ...
        st.session_state["logs"] = {"inventario": inv_log, "transacciones": trans_log, "feedback": fb_log}
...
with tab_auditoria:
    ...
    st.subheader("Registros excluidos (outliers marcados)")
    outlier_flag_cols = [c for c in df.columns if c.endswith("_outlier")]
    if outlier_flag_cols:
        flag = st.selectbox("Ver registros excluidos por", outlier_flag_cols)
        if st.checkbox("Mostrar solo registros marcados como outlier", value=True):
            st.dataframe(df[df[flag]], use_container_width=True)
        else:
            st.dataframe(df, use_container_width=True)
    else:
        st.info("No hay columnas de outliers disponibles en la vista filtrada actual.")
